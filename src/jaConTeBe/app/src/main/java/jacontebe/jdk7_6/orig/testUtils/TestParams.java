...
public class TestParams {
    ...
    public static String defaultSecurityManager = "java.rmi.RMISecurityManager";

    /* Initalize commonly used strings */
    static {
        try {
            testSrc = System.getProperty("user.dir");
            testClasses = TestLibrary.getProperty("test.classes", ".");

            // if policy file already set use it
            defaultPolicy = TestLibrary.getProperty("java.security.policy",
                    defaultPolicy);
            ...
            defaultSecurityManager = TestLibrary.getProperty(
                    "java.security.manager", defaultSecurityManager);

            defaultRmidPolicy = testSrc + File.separatorChar
                    + "rmid.security.policy";

            defaultGroupPolicy = testSrc + File.separatorChar
                    + "group.security.policy";

        } catch (SecurityException se) {
            TestLibrary.bomb("Security exception received"
                    + " during test initialization:", se);
        }
    }
}
