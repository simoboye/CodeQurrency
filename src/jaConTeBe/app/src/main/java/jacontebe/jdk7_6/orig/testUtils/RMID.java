...
public class RMID extends JavaVM {
    ...
    /**
     * Routine that creates an rmid that will run with or without a policy file.
     */
    ...
    public static RMID createRMID(OutputStream out, OutputStream err,
            boolean debugExec) {
        return createRMID(out, err, debugExec, true, TestLibrary.RMID_PORT);
    }

    public static RMID createRMID(OutputStream out, OutputStream err,
            boolean debugExec, boolean includePortArg, int port) {
        String options = makeOptions(debugExec);
        String args = makeArgs(includePortArg, port);
        RMID rmid = new RMID("sun.rmi.server.Activation", options, args, out,
                err, port);
    ...
    public static void removeLog() {
        /*
         * Remove previous log file directory before starting up rmid.
         */
    ...
    /**
     * This method is used for adding arguments to rmid (not its VM) for passing
     * as VM options to its child group VMs. Returns the extra command line
     * arguments required to turn on jcov code coverage analysis for rmid child
     * VMs.
     */
    ...
    /*
     * public void slowStart() throws IOException { start(60000); }
     */

    public void start(long waitTime, String loc) throws IOException {
        ...
        int slopFactor = 1;
        try {
            slopFactor = Integer.valueOf(TestLibrary.getExtraProperty(
                    "jcov.sleep.multiplier", "1"));
    ...
    /*
     * public void restart() throws IOException { destroy(); start(); }
     */
    /**
     * Ask rmid to shutdown gracefully using a remote method call. catch any
     * errors that might occur from rmid not being present at time of shutdown
     * invocation.
     * 
     * Shutdown does not nullify possible references to the rmid process object
     * (destroy does though).
     */
    ...
    public static void shutdown(int port) {
        ...
        try {
            ...
            try {
                ...
                system = (ActivationSystem) Naming.lookup("//:" + port
    ...
    /**
     * Ask rmid to shutdown gracefully but then destroy the rmid process if it
     * does not exit by itself. This method only works if rmid is a child
     * process of the current VM.
     */
    public void destroy() {
