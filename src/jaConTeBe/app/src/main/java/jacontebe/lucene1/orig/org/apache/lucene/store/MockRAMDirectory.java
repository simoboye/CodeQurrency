...
public class MockRAMDirectory extends RAMDirectory {
    ...
    /**
     * If set to true, we throw an IOException if the same file is opened by
     * createOutput, ever.
     */
    ...
    private synchronized void deleteFile(String name, boolean forced)
            throws IOException {
        ...
        if (!forced) {
            if (noDeleteOpenFile && openFiles.containsKey(name)) {
                throw new IOException("MockRAMDirectory: file \"" + name
    ...
    /**
     * Like getRecomputedSizeInBytes(), but, uses actual file lengths rather
     * than buffer allocations (which are quantized up to nearest
    ...
    public synchronized void close() {
        ...
        if (noDeleteOpenFile && openFiles.size() > 0) {
            ...
            throw new RuntimeException(
                    "MockRAMDirectory: cannot close: there are still open files: "
                            + openFiles);
        }
    }

    /**
     * Objects that represent fail-able conditions. Objects of a derived class
     * are created and registered with the mock directory. After register, each
     * object will be invoked once for each first write of a file, giving the
     * object a chance to throw an IOException.
     */
    public static class Failure {
         ...
         * one failure object and then reuse it in multiple cases. This,
         * combined with the fact that Failure subclasses are often anonymous
         * classes makes reset difficult to do otherwise.
         * 
