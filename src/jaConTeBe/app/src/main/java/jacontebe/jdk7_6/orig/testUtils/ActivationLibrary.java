...
public class ActivationLibrary {
     ...
     * 
     * This method intentionally avoids performing a lookup on the activation
     * system.
     */
        ...
        for (int i = 0; i < 15; i++) {
            ...
            try {
                ...
                mesg("caught an exception trying to"
                        + " start rmid, last exception was: " + e.getMessage());
                e.printStackTrace();
            }
        }
        return false;
     ...
     */
    private static boolean containsString(String[] strings, String contained) {
        if (strings == null) {
            if (contained == null) {
                return true;
            }
        ...
        for (int i = 0; i < strings.length; i++) {
            if ((strings[i] != null) && (strings[i].indexOf(contained) >= 0)) {
                return true;
            }
        }
        return false;
    ...
    public static void rmidCleanup(RMID rmid, int port) {
        if (rmid != null) {
            if (!ActivationLibrary.safeDestroy(rmid, port, SAFE_WAIT_TIME)) {
                TestLibrary.bomb("rmid not destroyed in: " + SAFE_WAIT_TIME
                        + " milliseconds");
            }
        }
        RMID.removeLog();
    ...
    /**
     * Invoke shutdown on rmid in a way that will not cause a test to hang.
     * 
     * @return whether or not shutdown completed succesfully in the timeAllowed
     */
